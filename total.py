...
df_S = pd.DataFrame(data=S_data,index=[format(800),format(1000),format(1200)], columns=('LBDD','Single Line shift', 'LARCMS','Proposed scheme'))
...
plt.yticks([i for i in np.arange(0,max(map(max, S_data))+500, 2500)],[i if i < 10000 else format(i,",") for i in np.arange(0,int(max(map(max, S_data))+500), 2500)])
